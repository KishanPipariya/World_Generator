...
def test_env_loading_sqlite_path(tmp_path):
    env_content = """
WORLDWRIGHT_SQLITE_PATH=/tmp/worldwright-test.sqlite3
    """
    ...
    with patch.dict(os.environ, {}, clear=True), patch("app.config.load_dotenv") as mock_dotenv:
        ...
        def side_effect(*args, **kwargs):
            os.environ["WORLDWRIGHT_SQLITE_PATH"] = "/tmp/worldwright-test.sqlite3"
            
        mock_dotenv.side_effect = side_effect
        settings = load_settings()
        
        assert settings.sqlite_path == "/tmp/worldwright-test.sqlite3"
